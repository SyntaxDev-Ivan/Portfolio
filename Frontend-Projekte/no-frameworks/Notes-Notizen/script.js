...
function load() {
  ...
  function addAllEventListener() {
    ...
    document.querySelector(".finished")
      .addEventListener("click", finishedAddingNote)

    document.querySelectorAll(".finishEdit")
      .forEach(elm => {
  ...
  function saveEdit(ev) {
    ...
    let value = document.querySelector(`.a${editClass[editClass.length - 1]}`).value
  ...
  function deleteNote(ev) {
    ...
    notesList = updateIds([...firstArr, ...secondArr])
    listNotes()
    addAllEventListener()
  }
  ...
  function addNote(ev) {
    document.querySelector(".add-overlay")
      .style.display = "flex"
  }
  ...
  function cancel(ev) {
    ...
    document.querySelectorAll(".overlay textarea")
      .forEach(elm => {
        elm.value = ""
      })
  }
  ...
  function listNotes() {
    ...
    for (let i = 0; i < notesList.length; i++) {
      notes.innerHTML += `
      <div class="note" id=${notesList[i].id}>
      <div class="note-header">
        <div class="edit"><img
            class="edit-${notesList[i].id}"
      ...
      <div class="note-body">
        <textarea class="a${notesList[i].id}" disabled="true">${notesList[i].value}</textarea>
      </div>
    </div>
      `
    }
